...
class ItemBottomSheet1 extends StatefulWidget {
  final Item? item;
  final bool isCampaign;
  final CartModel? cart;
  final int? cartIndex;
  final bool inStorePage;
  const ItemBottomSheet1({super.key, required this.item, this.isCampaign = false, this.cart, this.cartIndex, this.inStorePage = false});

  @override
  State<ItemBottomSheet1> createState() => _ItemBottomSheetState();
}

class _ItemBottomSheetState extends State<ItemBottomSheet1> {
               ...
               (!widget.item!.scheduleOrder! && !isAvailable) ? const SizedBox() : Container(
                  ...
                  child: Column(children: [
                    ...
                    SafeArea(
                      child: Row(children: [
                          // Quantity
                      !widget.item!.foodVariations!.any((element) => element.multiSelect == true) ?     Row(
                        ...
                        children: [
                          ...
                          Expanded(child: GetBuilder<CartController>(
                            builder: (cartController) {
                              bool hasMultiSelectrue = widget.item!.foodVariations!.any((element) => element.multiSelect == true);
                              return CustomButton(
                                ...
                                onPressed:
                                 ...
                                 (Get.find<SplashController>().configModel!.moduleConfig!.module!.stock! && stock! <= 0) && hasMultiSelectrue ? null : () async  {
                                  String? invalid;
                                  if(_newVariation) {
                                    for(int index=0; index<widget.item!.foodVariations!.length; index++) {
                      ...
                      child: Row(children: [
                        ...
                        Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                          ...
                          Checkbox(
                            ...
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(Dimensions.radiusSmall)),
                            onChanged:(bool? newValue) {
                              print("${itemController.selectedVariations[index][i]}");
                              itemController.setNewCartVariationIndex(index, i, item!);
                            },
                            visualDensity: const VisualDensity(horizontal: -3, vertical: -3),
                            side: BorderSide(width: 2, color: Theme.of(context).hintColor),
