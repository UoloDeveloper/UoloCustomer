...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    ...
    // Initialize selectedUrl based on conditions
    ...
    await browser.openUrlRequest(
      ...
      settings: InAppBrowserClassSettings(
        webViewSettings: InAppWebViewSettings(
          ...
          // isWebViewInspectable: kDebugMode,
          disableDefaultErrorPage: true,
          hardwareAcceleration: false, // Disable hardware acceleration to mitigate GPUAUX errors
...
class MyInAppBrowser extends InAppBrowser {
  ...
  void onLoadError(url, code, message) {
    ...
    if (message.contains('net::ERR_UNKNOWN_URL_SCHEME') && url.toString().startsWith('upi://')) {
      Get.snackbar('Info', 'Opening UPI app...');
    } else {
  ...
  Future<NavigationActionPolicy> shouldOverrideUrlLoading(navigationAction) async {
    ...
    if (url.startsWith('upi://')) {
      try {
        ...
        if (kDebugMode) {
          print("Failed to launch UPI URL: $e");
        }
        Get.snackbar('Error', 'Could not open UPI app. Please ensure a UPI app is installed.');
        return NavigationActionPolicy.CANCEL;
      }
    }

    return NavigationActionPolicy.ALLOW; // Allow other URLs to load normally
  }
